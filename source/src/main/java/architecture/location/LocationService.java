...
public class LocationService {
	...
	private static List<State> getStates(final List<FlatLocation> locations, final Country country) {
		return locations.stream()
			.filter(location -> location.getCountry().getCode().equals(country.getCode()))
			.map(FlatLocation::getState)
			.distinct()
			.map(state -> {
	...
	private static List<City> getCities(final List<FlatLocation> locations, final State state) {
		return locations.stream()
			.filter(location -> location.getState().getCode().equals(state.getCode()))
			.map(FlatLocation::getCity)
			.distinct()
			.map(city -> {

...
class GroupServiceTest {
	...
	void testGroupPropertiesByDifferentDates() {
		...
		Assertions.assertEquals(person.getPassport(),
				groups.get(person.getPassportExpirationDate()).values().toArray()[0]);
		Assertions.assertEquals(person.getDriverLicense(),
				groups.get(person.getDriverLicenseExpirationDate()).values().toArray()[0]);
	}

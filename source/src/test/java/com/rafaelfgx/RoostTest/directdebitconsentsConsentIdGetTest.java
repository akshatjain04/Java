...
public class directdebitconsentsConsentIdGetTest {
	...
	public void setUp() {
		...
		envList = dataloader.load(
				"src\\test\\java\\com\\rafaelfgx\\RoostTest\\direct-debit-consents_consentIdGetTest.csv", envVarsList);
	}
          ...
          RestAssured.baseURI = (testData.get("BASE_URL") != null && !testData.get("BASE_URL").isEmpty()) ? testData.get("BASE_URL"): "https://hsbcdeveloperportalpreprod.digitalapicraft.com:8085/https://sandbox.ob.business.hsbc.com.hk/mock/open-banking/v1.0/direct-debit";
              ...
              if (testData.get("statusCode") != null) {
                MatcherAssert.assertThat(responseObj.statusCode(),
                 ...
                 List<Integer> expectedStatusCodes = Arrays.asList(200,400,401,403,405,406,429,5XX);
          ...
          case 411:
           ...
           break;
              ...
              if (response.get("errors["+ i +"].extendedDetails.path") != null) {
				...
				}System.out.println("Description: Unauthorised");}System.out.println("Description: Forbidden");}System.out.println("Description: Method Not Allowed");

	if(response.get("id")!=null){MatcherAssert.assertThat(response.get("id"),instanceOf(String.class));MatcherAssert.assertThat(response.getString("id").length(),lessThanOrEqualTo(40));

	MatcherAssert.assertThat(response.getString("id").length(),greaterThanOrEqualTo(1));

	}

	if(response.get("errors")!=null){for(

	int i = 0;

	i<response.getList("errors").size();i++)
	{
		if (response.get("errors[" + i + "].code") != null) {
			MatcherAssert.assertThat(response.get("errors[" + i + "].code"), instanceOf(String.class));
		}
		...
		if (response.get("errors[" + i + "].extendedDetails") != null) {
			if (response.get("errors[" + i + "].extendedDetails.path") != null) {
				...
				MatcherAssert.assertThat(response.getString("errors[" + i + "].extendedDetails.path").length(),
	...
	}MatcherAssert.assertThat(response.getList("errors"),
	...
	if(response.get("message")!=null){MatcherAssert.assertThat(response.get("message"),instanceOf(String.class));}}System.out.println("Description: Internal Server Error");

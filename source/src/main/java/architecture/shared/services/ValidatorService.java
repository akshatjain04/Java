...
public class ValidatorService {
	...
	public boolean validateCpf(String value) {
		...
		return value.matches("\\d{11}") && !value.matches("(\\d)\\1{10}") && cpfVerifier(value, 9)
				&& cpfVerifier(value, 10);
	}

	private static boolean cpfVerifier(CharSequence value, int length) {
		final var verifier = IntStream.range(0, length)
			.map(i -> Character.getNumericValue(value.charAt(i)) * (length + 1 - i))
			.sum() * 10 % 11 % 10;
	...
	public boolean validateCnpj(String value) {
		value = value.replaceAll("\\D", "");
		return value.matches("\\d{14}") && !value.matches("(\\d)\\1{13}") && cnpjVerifier(value, 12)
				&& cnpjVerifier(value, 13);
	}
	...
	private static boolean cnpjVerifier(CharSequence value, int index) {
		...
		if (index == 12)
			weights.remove(0);
		final var sum = IntStream.range(0, index)
			.map(i -> Character.getNumericValue(value.charAt(i)) * weights.get(i))
			.sum();
